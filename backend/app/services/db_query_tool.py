...
def build_tool_schema(connection_names: list[str], has_mongo: bool, has_sql: bool) -> dict:
    ...
    if has_sql:
        query_desc.append(
            ...
            'را برای محاسبه دستی برنگردان. اگر نام جدول در اسکیما به‌صورت سه‌بخشی '
            '«دیتابیس.اسکیما.جدول» آمده (یعنی این اتصال چند دیتابیس را هم‌زمان پوشش می‌دهد)، '
            'همان نام کامل سه‌بخشی را دقیقاً همان‌طور در FROM/JOIN بنویس؛ این‌طور می‌توانی '
            'حتی بین دو دیتابیس مختلف روی همین سرور هم JOIN بزنی.'
        )
    if has_mongo:
        query_desc.append(
