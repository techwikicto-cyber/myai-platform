...
def _to_out(row: ModelSettings) -> ModelSettingsOut:
    ...
    return ModelSettingsOut(
        ...
        reviewer_llm_base_url=row.reviewer_llm_base_url,
        reviewer_llm_model=row.reviewer_llm_model,
        reviewer_llm_api_key_set=bool(row.reviewer_llm_api_key_encrypted),
    )
...
async def update_model_settings(payload: ModelSettingsIn, db: AsyncSession = Depends(get_db)):
    ...
    row.reviewer_llm_base_url = payload.reviewer_llm_base_url
    row.reviewer_llm_model = payload.reviewer_llm_model
    if payload.reviewer_llm_api_key:
        row.reviewer_llm_api_key_encrypted = encrypt_secret(payload.reviewer_llm_api_key)
    await db.commit()
    await db.refresh(row)
    return _to_out(row)
...
@router.post("/model/test-reviewer-llm", response_model=ConnectionTestResult)
async def test_reviewer_llm(payload: ModelSettingsIn | None = None, db: AsyncSession = Depends(get_db)):
    if payload and payload.reviewer_llm_base_url:
        row = await _get_or_create_row(db)
        api_key = payload.reviewer_llm_api_key or (
            decrypt_secret(row.reviewer_llm_api_key_encrypted) if row.reviewer_llm_api_key_encrypted else ""
        )
        config = LlmConfig(
            base_url=payload.reviewer_llm_base_url, api_key=api_key, model=payload.reviewer_llm_model or "",
        )
    else:
        config = await get_reviewer_llm_config(db)
        if config is None:
            return ConnectionTestResult(success=False, message="مدل بازبین هنوز تنظیم نشده است")
    success, message = await test_llm_connection(config)
    return ConnectionTestResult(success=success, message=message)
