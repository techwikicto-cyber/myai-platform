...
export default function ProfileModal({ forced = false, onClose }: ProfileModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-[#1e1e2e] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between border-b border-white/5 p-4 bg-white/5">
          ...
          {!forced && (
            <button onClick={onClose} className="rounded-md p-1.5 text-gray-400 hover:bg-white/10 hover:text-white transition-colors">
              <IconX />
            </button>
          )}
        </div>
          ...
          <div className="flex border-b border-white/5">
            <button
              ...
              className={`flex-1 py-3 text-sm font-medium transition-colors ${
                activeTab === 'profile' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
            >
              پروفایل
            </button>
            <button
              ...
              className={`flex-1 py-3 text-sm font-medium transition-colors ${
                activeTab === 'password' ? 'text-primary border-b-2 border-primary bg-primary/5' : 'text-gray-400 hover:text-white hover:bg-white/5'
              }`}
            >
              تغییر رمز عبور
            </button>
        ...
        <div className="p-5 overflow-y-auto">
          {activeTab === 'profile' && !forced && (
            <div className="space-y-6">
              <div className="flex flex-col items-center gap-3">
                <div 
                  className="relative size-24 rounded-full border-2 border-white/10 bg-black/20 overflow-hidden cursor-pointer group flex items-center justify-center"
                  onClick={() => fileInputRef.current?.click()}
                >
                  {avatar ? (
                    <img src={avatar} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-3xl text-gray-500 font-bold">{user?.email[0].toUpperCase()}</span>
                  )}
                ...
                <p className="text-xs text-gray-400 text-center">حجم مجاز: ۵۰ کیلوبایت (خودکار فشرده می‌شود)</p>
              </div>

              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">نام و نام خانوادگی</label>
                  <input
                    type="text"
                    value={fullName}
                    onChange={(e) => setFullName(e.target.value)}
                    className="w-full rounded-lg border border-white/10 bg-black/20 px-3 py-2 text-sm text-white placeholder-gray-500 outline-none focus:border-primary focus:ring-1 focus:ring-primary"
                    placeholder="مثال: علی رضایی"
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">ایمیل (غیرقابل تغییر)</label>
                  <input
                    type="text"
                    value={user?.email || ''}
                    disabled
                    className="w-full rounded-lg border border-white/5 bg-white/5 px-3 py-2 text-sm text-gray-400 outline-none"
                    dir="ltr"
                  />
                </div>
              </div>
          ...
          {activeTab === 'password' && (
            <div className="space-y-4">
              {forced && (
                <div className="mb-4 rounded-lg bg-blue-500/10 p-3 border border-blue-500/20 text-sm text-blue-200 leading-relaxed">
